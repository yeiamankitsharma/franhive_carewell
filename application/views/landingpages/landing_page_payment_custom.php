...
<head>
	...
	<title><?= htmlspecialchars($landing_page['LANDING_PAGE_NAME']); ?></title> <?php
...
?>
	...
	#payment-block .rowline+.rowline {
		border-top: 1px dashed #e5e7eb;
	}

	#payment-block .price-big {
		font-size: 1.45rem;
		font-weight: 700;
	}

	#payment-block .strike {
		text-decoration: line-through;
		color: #9ca3af;
	}

	#payment-block .discount-row {
		color: var(--accent-2);
	}
	...
	#payment-block .btn-apply:hover {
		background: #f9fafb;
	}

	#payment-block .btn-apply:active {
		transform: translateY(1px);
	}

	#payment-block .coupon-help {
		font-size: .85rem;
		color: var(--muted);
		margin-top: 6px;
	}
	...
	#payment-block .promo-nudge .burst svg {
		width: 16px;
		height: 16px;
	}

	#payment-block .promo-nudge .highlight {
		font-weight: 700;
	}

	#payment-block .promo-nudge .sub {
		color: #0f5132;
		opacity: .9;
	}
	...
	#payment-block .btn-accent:active {
		transform: translateY(1px);
	}

	#payment-block .btn-accent .btn-title {
		font-size: 1rem;
	}

	#payment-block .btn-accent .price-wrap {
		display: flex;
	...
	#payment-block .btn-accent .price-wrap .final {
		font-weight: 800;
		font-size: 1.1rem;
	}
	...
	#faqAccordion .btn-faq .small {
		color: #6b7280;
		font-weight: 600;
	}

	#faqAccordion .btn-faq:focus {
		box-shadow: none;
	}
	...
	#faqAccordion .collapsed .chev {
		transform: rotate(0deg);
	}

	#faqAccordion .chev.rot {
  ...
  color:#ffffff; padding:18px; display:flex; align-items:center; gap:12px;
	...
	#register-block .reg-sub {
		margin: 2px 0 0;
		opacity: .95;
		font-weight: 600;
	}

	#register-block .reg-body {
		padding: 18px;
	}

	#register-block .form-row-gap {
		display: grid;
	...
	#register-block .btn-reg:hover {
		background: #059669 !important;
		box-shadow: 0 8px 18px rgba(5, 150, 105, .28);
	}

	#register-block .btn-reg:active {
		transform: translateY(1px);
	}

	#register-block .btn-reg .chev {
		margin-left: 6px;
		font-weight: 900;
	}
	...
	#coach .coach-intro .col-md-8 {
		display: flex;
		flex-direction: column;
	}

	#coach .coach-intro .coach-name {
		margin-top: 2px;
	}
	...
	#help-connect .connect-sub {
		color: #374151;
	}

	#help-connect .connect-cta {
		display: flex;
		flex-wrap: wrap;
		gap: 10px;
		margin-top: 12px;
	...
	#help-connect .chip:hover {
		background: #f9fafb;
	}

	#help-connect .chip:active {
		transform: translateY(1px);
	}

	#help-connect .chip svg {
		width: 18px;
		height: 18px;
	}

	/* brand dots */
	#help-connect .dot-msg {
		color: #0084FF;
	}
	...
	#payment-block #pay-now .cta-price {
		display: none;
		/* hidden by default, shown in deposit mode */
		align-items: center;
		justify-content: center;
		gap: 8px;
		margin-top: 4px;
	...
	#payment-block #pay-now .sep {
		opacity: .5;
	}

	#payment-block #pay-now .muted {
		color: #6b7280;
	}
	...
	@media (min-width: 992px) {
		...
		/* keep single column on narrow sidebar */
	}
	...
	/* keep Stripe button full-width inside grid cell */
	.btn-bank .btn-title,
	#pay-now .btn-title {
		font-weight: 800;
	}
	...
	span#bank-pay-remaining-text {
		...
		/* <- italic */
		font-weight: 400;
		/* keep readable (you had 'initial') */
		text-shadow:
			0 1px 0 rgba(0, 0, 0, .25),
			/* slight bottom edge */
			0 2px 4px rgba(0, 0, 0, .2);
		/* soft glow for contrast */
	}
	...
	.bundle-logo {
		max-height: 60px;
		/* keeps both logos uniform in height */
		margin: 0 10px;
	...
	<div class="container">
		...
		<section class="hero-split my-3 with-halo">
			<div class="row align-items-center">
				<div class="col-md-6">
					...
					<p class="hero-lead mb-2"> A rare fusion where the science of the mind meets the healing of the soul. Earn <strong>dual trainings</strong> in Global NLP Practitioner Training and Universal Life Force Activation (Reiki). Live, practice-driven, and transformation-focused. </p>
					<ul class="mini-benefits">
						...
						<?= $landing_page['LANDING_PAGE_MAIN_CONTENT']; ?>
					<div class="hero-cta">
						<a href="#payment-block" class="btn btn-hero">Enroll now</a>
						<a href="#faqAccordion" class="btn btn-outline-hero">View FAQs</a>
					</div>
				</div>
				<div class="col-md-6"> <?php if (!empty($landing_page['BANNER_IMAGE'])): ?> <div class="hero-image-wrap mt-3 mt-md-0">
		...
		<div class="content">
			...
			<div class="main-content">
						...
						<div class="row coach-intro align-items-start">
							<div class="col-md-4 mb-3 mb-md-0">
								<div class="photo-wrap">
									<img src="<?= $landing_page['COACH_PHOTO']; ?>" alt="Barinderjeet Kaur — Empower Your Destiny" class="coach-img">
								</div>
							</div>
							<div class="col-md-8">
								<div class="coach-name"><?= $landing_page['COACH_NAME']; ?></div>
								<div class="coach-sub"><?= $landing_page['COACH_SUBTITLE']; ?></div>
								<!-- Updated short intro -->
								<p class="coach-text mb-2"> <?= $landing_page['COACH_BIO_SHORT']; ?> </p>
							</div>
						</div>
						<!-- Row 2: Full-width continuation + bullets + CTAs -->
						<div class="coach-details">
							<div class="row">
								<div class="col-12"> <?= $landing_page['COACH_BIO_LONG']; ?>
				...
				<!-- FAQ (collapsed by default) --> <?php
        // Example: $landing_page comes from your model row
        $faqJsonString = isset($landing_page['FAQ_JSON']) ? $landing_page['FAQ_JSON'] : '[]';
        $faqs = json_decode($faqJsonString, true);

        // If JSON decode failed, fall back to empty array and (optionally) log
        if (!is_array($faqs)) {
        ...
        if (!function_exists('e')) {
            function e($str) { return html_escape($str, true); } // CI3 html_escape
        }
        ?> <div class="duo-section">
					<div class="duo-h2">FAQ — your questions, answered</div>
					<div id="faqAccordion" class="accordion"> <?php if (empty($faqs)): ?> <div class="card">
							<div class="card-header">
								<div class="card-body">No FAQs available right now.</div>
							</div>
						</div> <?php else: ?> <?php foreach ($faqs as $i => $item): 
                    // Ensure keys exist
              ...
              ?> <div class="card">
							<div class="card-header" id="<?= e($hid) ?>">
								<button class="btn btn-link btn-faq collapsed" type="button" data-toggle="collapse" data-target="#<?= e($cid) ?>" aria-expanded="false" aria-controls="<?= e($cid) ?>"> <?= e($q) ?> <span class="chev collapsed">⌄</span>
								</button>
							</div>
...
?> <h2 class="text-center text-brandcolor">Testimonials</h2>
				<div class="testimonials-list"> <?php
    ...
    ?> <div class="testimonial-video">
						<iframe src="<?= htmlspecialchars($embed) ?>" title="Testimonial video" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe>
					</div> <?php
        endif;
      endforeach;
    ?> <?php if ($hasImages): ?> <?php foreach (explode(',', $landing_page['TESTIMONIALS_IMAGES']) as $image): $image = trim($image); if(!$image) continue; ?> <div class="testimonial-image">
						<img src="<?= base_url().htmlspecialchars($image) ?>" class="img-fluid w-100" alt="Testimonial Image">
					</div> <?php endforeach; ?> <?php endif; ?> <?php
      // --- Videos first (YouTube Shorts → embed) ---
      foreach ($videos as $url):
        $url = trim($url);
        if (!$url) continue;
			...
			<div class="regcard">
				...
				<div class="reg-body">
					<form id="contactForm">
						...
						<!-- Privacy note -->
						<div class="privacy"> Your details are kept private and used only to contact you about this training. </div>
						<!-- Actions (keeps your PHP button text logic) -->
						<div class="actions"> <?php if ($landing_page['LANDING_PAGE_ID'] == 20) { ?> <button type="submit" class="btn-reg"> YES! I Want to Activate My Relationships <span class="chev">›</span>
							</button> <?php } else { ?> <button type="submit" class="btn-reg"> Submit <span class="chev">›</span>
							</button> <?php } ?> </div>
					</form>
				</div>
			</div>
			...
			<div class="card pcard">
				...
				<div class="pcard-body">
					...
					<div class="rowline">
						...
						<div><strong id="item-nlp" data-amount-cents="221400" data-currency="aud">AUD$<?= $landing_page['PRICE_FIRST_TRAINING'];?></strong></div>
					</div>
					<div class="rowline">
						<div>Individual: <strong>Training-2</strong></div>
						<div><strong id="item-reiki" data-amount-cents="88800" data-currency="aud">AUD$<?= $landing_page['PRICE_SECOND_TRAINING'];?></strong></div>
					</div>
					...
					<div class="rowline" style="border-top:1px solid #eef2f7">
						...
						<div><strong id="total-price"><?= $landing_page['PRICE_BUNDLE_TOTAL'];?></strong></div>
					</div>
					<!-- Coupon -->
                ...
                <div>
                  ...
                  <div class="sub small">Limited-time offer • Discounts shown before you pay</div>
					...
					<div id="coupon-nudge" class="promo-nudge emphasis" role="note" aria-live="polite">
						<div class="burst" aria-hidden="true">
							<svg viewBox="0 0 24 24" fill="none">
								<path d="M12 2l2.6 5.5 6 .5-4.6 4 1.4 5.8L12 15.9 6.6 17.8 8 12 3.4 8l6-.5L12 2z" fill="#ffffff" /></svg>
						</div>
						<div>
							<div class="headline">Bundle both courses & save up to 50%</div>
							<div class="sub"> <?= $landing_page['VIP_CARD_TEXT'];?> <a href="#coupon" id="apply-bundle" class="cta-link">Apply now</a>
							</div>
						</div>
					</div>
					...
					<div class="btn-row">
						<div class="pay-grid">
							...
							<button id="pay-bank" class="btn btn-bank" type="button" aria-live="polite">
								...
								<div class="cta-price">
									<span class="pill-amount" id="bank-pay-deposit">AUD$<?= $landing_page['PRICE_DEPOSIT'];?></span>
									<span class="sep">•</span>
									<span class="muted small"> Remaining <strong id="bank-pay-remaining">AUD$<?= $landing_page['PRICE_BUNDLE_TOTAL'] - $landing_page['PRICE_DEPOSIT'];?></strong> later <span id="bank-pay-remaining-text"> (** Apply your coupon to get 50% off)</span>
									</span>
								</div>
							</button>
							<!-- Stripe (unchanged id so your JS continues to work) -->
							<button id="pay-now" class="btn btn-accent" type="button" aria-live="polite">
								<div class="btn-title" id="pay-now-label">Secure Online Checkout AUD$<?= $landing_page['PRICE_DEPOSIT'];?> deposit</div>
								<div class="cta-price">
									<span class="pill-amount" id="pay-now-deposit">AUD$<?= $landing_page['PRICE_DEPOSIT'];?></span>
									<span class="sep">•</span>
									<span class="muted small"> Remaining <strong id="pay-now-remaining">AUD$<?= $landing_page['PRICE_BUNDLE_TOTAL'] - $landing_page['PRICE_DEPOSIT'];?></strong> later </span>
								</div>
								...
								<div class="modal-body">
									...
									<div class="d-flex justify-content-between align-items-center bg-light rounded p-2 mb-3">
										<div>
											...
											<div class="font-weight-bold" id="bank-modal-now">AUD$<?= $landing_page['PRICE_DEPOSIT'];?></div>
										</div>
										<div class="text-muted">•</div>
										<div>
											<div class="small text-uppercase text-muted">Balance later</div>
											<div class="font-weight-bold" id="bank-modal-later">AUD$<?= $landing_page['PRICE_BUNDLE_TOTAL'] - $landing_page['PRICE_DEPOSIT'];?></div>
										</div>
									</div>
	...
	// ---- globals used across functions ----
	const DEPOSIT_CENTS = <?= $landing_page['PRICE_DEPOSIT']*100;?> // AUD$500 deposit
	const BUNDLE_TOTAL_CENTS =<?= $landing_page['PRICE_BUNDLE_TOTAL']*100;?>; // AUD$1,551.00 (bundle after coupon)
	const locale = 'en-AU';
	const currency = 'aud';
	...
	function updatePayCta(totalCents) {
		...
		if(totalCents === BUNDLE_TOTAL_CENTS) {
			$("#bank-pay-label").text("Direct Bank Transfer — AUD$"+DEPOSIT_CENTS+" deposit");
		} else {
	...
	$(function() {
		...
		$applyBtn.on("click", function() {
			...
			$.ajax({
				...
				if(resp && resp.status === "success") {
					...
					$couponFeedback.html('<span class="text-success">Coupon applied: ' + resp.data.code + ' (−' + money(resp.data.discount_cents) + ')</span>');
		...
		$("#pay-now").off("click.eyd").on("click.eyd", function() {
			...
			const endpoint = isDepositMode ? "<?= base_url('/PaymentController/create_checkout_session_deposit') ?>" : "<?= base_url('/PaymentController/create_checkout_session') ?>";
		...
		$form.on("submit", function(e) {
			...
			$.ajax({
				...
				if(resp && resp.status === "success") {
					...
					$status.addClass("alert alert-success").html(<?= json_encode($landing_page['LANDING_PAGE_THANKS_CONTENT']); ?>);
					$form.slideUp(200);
				} else {
	...
	(function() {
		...
		window.currentTotalCents = (typeof window.currentTotalCents !== "undefined") ? window.currentTotalCents : (typeof baseTotalCents === "function" ? baseTotalCents() : 155100); // AUD$1,551.00
		...
		if(typeof updatePayCta === "function") {
			...
			window.updatePayCta = function(totalCents) {
				...
				$("#bank-pay-label").text(totalCents === BUNDLE_TOTAL_CENTS ? "Bank transfer — AUD$"+DEPOSIT_CENTS/100+" deposit" : "Bank transfer — Pay " + (typeof moneyAU === "function" ? moneyAU(totalCents) : ("AUD$" + (totalCents / 100).toFixed(2))));
			};
			// initialize once
